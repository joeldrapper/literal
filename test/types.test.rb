...
test "_Frozen" do
	...
	assert _Frozen(Numeric) >= _Frozen(Integer)
	refute _Frozen(Integer) >= _Frozen(Numeric)
	assert _Frozen(Integer) >= _Frozen(Integer)
	assert _Frozen(Numeric) >= _Constraint(Integer, frozen?: true)
end
